...
# REMEDIATED: Enabled block_public_acls and block_public_policy to prevent public write access
...
# REMEDIATED: Changed from public-read-write to private to block public write access
...
# REMEDIATED: Removed write permissions (PutObject, DeleteObject) to block public write access
...
output "security_warnings" {
  value = "WARNING: Public write access blocked, but other misconfigurations remain (no encryption, no versioning)!"
}
