...
# REMEDIATED: Public access block enabled to block public access
...
# REMEDIATED: Private ACL (no public read/write)
...
# REMEDIATED: Bucket policy with read-only access (no public write)
...
output "security_warnings" {
  value = "NOTE: Public write access has been blocked. Other misconfigurations (no encryption, no versioning) remain for testing purposes."
}
